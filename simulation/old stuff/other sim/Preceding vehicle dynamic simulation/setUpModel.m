...
precedingVehicleParams;

%% load the scene data files
% to remove in final version, keep until we see we don't need it
%load data from Driving Scenario Designer
%load('USHighway.mat');
%refPose = data.ActorSpecifications(1,46).Waypoints;

load('custom scenario.mat'); % load scenario file
load('custom waypoints.mat'); % load preceding vehicle waypoints
refPose = waypoints(:,1:2); % trajectory to be followed
...
psi_o = -0.157; % it's an important step to initialize yaw angle
