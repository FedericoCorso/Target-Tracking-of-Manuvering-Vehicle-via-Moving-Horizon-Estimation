...
if nargin<4 % check for the number of input arguments
    sideView = false;
end
...
if sideView
    ...
    hCarPlot = axes(hViewPanel);
    chasePlot(egoCar, 'Parent', hCarPlot, 'ViewHeight', 130, 'ViewLocation', [0 0], 'ViewPitch', 80);
end
...
if isa(sensor,'drivingRadarDataGenerator')
    plotCoverageArea(caPlotter, sensor.MountingLocation(1:2),...
        sensor.RangeLimits(2), sensor.MountingAngles(1), sensor.FieldOfView(1));
else
...
if isa(sensor,'visionDetectionGenerator') && strcmpi(sensor.DetectorOutput,'lanes and objects')
   ...
   laneBoundaryPlotter(bep, 'DisplayName','lane detections','Color','red');
else
