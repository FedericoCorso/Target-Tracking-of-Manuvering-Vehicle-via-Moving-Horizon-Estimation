...
%   
% OUTPUTS:
%   cost    = vector valued function F(x) such that f = F(x)'F(x)
%   p       =   value of non linear equality constraints
%   h       =   value of non linear inequality constraints
function out = target_cost_con_GN_multipleshooting(x,nz,M,Ts,Ts_sim,ymeas_camera,ymeas_radar,z_minus,R_camera,R_radar,Q,A_terminal,ego_pos,ego_or)
...
cost = [err_camera(1,:) err_camera(2,:) err_camera(3,:) err_camera(4,:)...
