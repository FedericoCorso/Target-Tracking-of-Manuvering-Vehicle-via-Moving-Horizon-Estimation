% Function executing MHE algorithm - with constant terminal cost
...
function [z_est,d_est,comp_time] = MHE_Con(Ts,Ts_sim,N,nz,ny,nu,Msec,MHE_interval,R_camera,R_radar,Q,A_terminal,z_minus,myoptions,ymeas_camera,ymeas_radar,a_max,a_min,alpha_max,alpha_min,ego_pos,ego_or)
...
for ind = M+1:MHE_interval:N
    tic
    myoptions.GN_funF       =  @(x)target_cost_GN_con(x,nz,M,Ts,Ts_sim,ymeas_camera(:,ind-M:ind),ymeas_radar(:,ind-M:ind),z_minus,R_camera,R_radar,Q,A_terminal,ego_pos(:,ind-M:ind),ego_or(:,ind-M:ind));
